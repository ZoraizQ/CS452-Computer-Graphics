...
private:
  ...
  struct SampleBuffer {
    ...
    void fill_color(int i, int j, Color c) {
      PixelColorStorage &pix = sub_pixels[i][j]; //unsigned char vector
    ...
    Color get_pixel_color() {
      ...
      for (uint y = 0; y < samples_per_side; y++) { // iterating through all subpixels, one by one
        for (uint x = 0; x < samples_per_side; x++) {
          avg_spcolor += sp_ratio * Color(sub_pixels[y][x].data()); // every subpixel contributes to the avg_spcolor by a ratio of 1/total
        }
      }
      return avg_spcolor;
    }
