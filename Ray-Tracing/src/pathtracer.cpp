...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) { // origin x,y integer coords in pixel space
  // compute amount of energy (spectrum) arriving on a pixel, brightness
  ...
  Spectrum output = Spectrum(0,0,0); // output is a Spectrum, can be converted into colors 
  int index = (int) ((y * sampleBuffer.w) + x); //(int) width of pixelBuffer used, get index of origin of ray in sampleBuffer
  
  if (num_samples == 1) { // if only one sample (camera ray), generate ray through center of pixel
    // ray generated from camera origin, x,y passed (location to camera)
    Ray r = camera->generate_ray((double) origin.x+0.5,(double) origin.y+0.5); // replace 1.0 with your values based on x,y,sampleBuffer.w,sampleBuffer.h
    r.depth = max_ray_depth; // ray depth set to maximum
    output = trace_ray(r, true); //trace this ray, output Spectrum
    sampleCountBuffer[index] = num_samples; //insert num_samples in origin's index in sample count buffer
  }
  else{
    // Part 1, Task 1:
    // Make a loop that generates num_samples camera rays and traces them
    // through the scene. Return the average Spectrum -- integral of the irradiance over this pixel, averaging over all ns_aa samples.
    // gridSampler in sampler.cpp - random samples
    // location to camera (x,y) scaled down
    for (int i = 0; i < num_samples; i++){
      Vector2D rndSample = gridSampler->get_sample(); // has x,y coords
      Ray r = camera->generate_ray((double) (origin.x+rndSample.x)/sampleBuffer.w, (double) (origin.y+rndSample.y)/sampleBuffer.h);
      r.depth = max_ray_depth; // ray depth set to maximum
      output += trace_ray(r, true); // add the traced spectrum from the ray to the output spectrum
    }
    //  inline Spectrum &operator/=(float s) so
    output /= (double) num_samples;
  }
  return output;
